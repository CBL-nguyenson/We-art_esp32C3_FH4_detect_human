...
struct SEAN_DEV_LED : Service::LightBulb {               // ON/OFF LED

  int ledPin;                                       // pin number defined for this LED
  SpanCharacteristic *power;                        // reference to the On Characteristic
  
  SEAN_DEV_LED(int ledPin) : Service::LightBulb(){       // constructor() method
...
struct SEAN_DEV_DimmableLED : Service::LightBulb {       // Dimmable LED
  ...
  SEAN_DEV_DimmableLED(int pin) : Service::LightBulb(){       // constructor() method
  ...
  boolean update(){                              // update() method
    ...
    ledPin->fade(power->getNewVal()*level->getNewVal(),2000,LedPin::PROPORTIONAL);
